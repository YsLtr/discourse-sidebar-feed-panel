...
- 刷新时会对比新旧列表，被取消公开、隐藏或删除而“凭空消失”的话题会保留原位并标记“话题异常”，点击可自行确认
...
信息流刷新时会和上一次的列表数据比对：一个本应还在当前列表范围内的话题，如果不再被服务器返回，就会保留在原位置并标记“话题异常”。这通常意味着它被取消公开（如被举报后临时隐藏）、被删除或被移入了受限板块，具体状态需要点击话题自行确认。

被标记的话题不会占用常规的保留数量，会留在原位置；当新进入列表的话题累计超过当前保留数量时自然移除，切换分类、排序或筛选触发的全量刷新也会清除。正常下沉到列表后面的话题不会触发这个标记。
